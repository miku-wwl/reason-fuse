...
import json
from evaluation.runtime import output_text
...
def postcondition(snapshot):
    ...
    if not matched:
        return 'UNKNOWN'
    return 'VERIFIED' if matched[-1].get('service_health') == 'HEALTHY' else 'FAILED'
...
def score(s, arm, records, *, lane, elapsed, scripted_requests=None, error=None):
    ...
    return {
        ...
        'admission_failure_count': sum(r.get('response', {}).get('status') == 'failed' for r in records),
